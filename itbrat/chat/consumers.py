...
class ChatConsumer(WebsocketConsumer):
    ...
    def receive(self, text_data=None, bytes_data=None):
        ...
        message, attachment, info = text_data_json["message"], text_data_json.get("attachment"), text_data_json["info"]
        ...
        message_serializer = dict(MessagesSerializer(_message, context={'request': self.scope["user"]}).data)
...
class ChatMessages(AsyncWebsocketConsumer):
    ...
    async def chat_message(self, event):
        ...
        sender = self.scope["user"]
        print(sender)
        if dict_to_be_sent['sender'] == sender.id:
            print('init')
